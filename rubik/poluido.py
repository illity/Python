...
for i in range(6):
 #print(int(i/2))
 #print(1+i%2-((1+i)%3==0)-((1+i)%5==0))
 t=np.array([canon[int(i/2)], canon[1+i%2-((1+i)%3==0)-((1+i)%5==0)]])
 for j in range(9):
  ...
  sq=(np.matmul(bsq,t))
  #.,.,y,x,.,z
  print(300*np.array(canon[int(i/2)]))
  #sq=translate(sq,300*np.array(canon[int(i/2)]))
  print(sq)
