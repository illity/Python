...
print(canon)
writer = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'xvid'), 30.0,(500,400))
for theta in range(2000):
 ...
 z=faces[:,4,:,2]
 for j in range(9):
   rect(faces[np.where(faces[:,4,:,0]==np.amax(faces[:,4,:,0]))[0][0],j],cor[np.where(faces[:,4,:,0]==np.amax(faces[:,4,:,0]))[0][0]][j])
   rect(faces[np.where(faces[:,4,:,1]==np.amin(faces[:,4,:,1]))[0][0],j],cor[np.where(faces[:,4,:,1]==np.amin(faces[:,4,:,1]))[0][0]][j])
   rect(faces[np.where(faces[:,4,:,2]==np.amin(faces[:,4,:,2]))[0][0],j],cor[np.where(faces[:,4,:,2]==np.amin(faces[:,4,:,2]))[0][0]][j])
