...
for i in range(100):
 print(i)
 for j in range(500):
  #code=code+'fill '+'{} 4 {} '.format(i,j)*2+cor(newimg[i][j])+'\n'
#open('serpico.mcfunction','w').write(code)
  open('serpico.mcfunction','a').write('fill '+'{} 4 {} '.format(i,j)*2+cor(newimg[i,j])+'\n')